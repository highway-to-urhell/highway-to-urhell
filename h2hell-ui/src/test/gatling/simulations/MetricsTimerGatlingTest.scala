...
/**
 * Performance test for the MetricsTimer entity.
 */
class MetricsTimerGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the MetricsTimer entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all metricsTimers")
            .get("/api/metrics-timers")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created metricsTimer")
                .get("${new_metricsTimer_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created metricsTimer")
            .delete("${new_metricsTimer_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
