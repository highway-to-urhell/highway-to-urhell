...
describe('EntryPointParameters e2e test', function () {
    ...
    it('should load EntryPointParameters', function () {
        entityMenu.click();
        element.all(by.css('[ui-sref="entry-point-parameters"]')).first().click().then(function() {
            element.all(by.css('h2')).first().getAttribute('data-translate').then(function (value) {
                expect(value).toMatch(/h2HellUiApp.entryPointParameters.home.title/);
            });
        });
    });
    ...
    it('should load create EntryPointParameters dialog', function () {
        element(by.css('[ui-sref="entry-point-parameters.new"]')).click().then(function() {
            element(by.css('h4.modal-title')).getAttribute('data-translate').then(function (value) {
                expect(value).toMatch(/h2HellUiApp.entryPointParameters.home.createOrEditLabel/);
            });
